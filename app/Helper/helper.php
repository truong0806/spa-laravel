...
function getSingleMedia($model, $collection = 'profile_image', $skip = true)
{
    ...
    if (getFileExistsCheck($media)) {
        ...
        switch ($collection) {
            case 'image_icon':
                $media = secure_asset('images/user/user.png');
                break;
            case 'profile_image':
                $media = secure_asset('images/user/user.png');
                break;
            case 'service_attachment':
                $media = secure_asset('images/default.png');
                break;
            case 'site_logo':
                $media = secure_asset('images/logo.png');
                break;
            case 'site_favicon':
                $media = secure_asset('images/favicon.png');
                break;
            case 'app_image':
                $media = secure_asset('images/frontend/mb-serv-1.png');
                break;
            case 'app_image_full':
                $media = secure_asset('images/frontend/mb-serv-full.png');
                break;
            case 'footer_logo':
                $media = secure_asset('landing-images/logo/logo.png');
                break;
            case 'logo':
                $media = secure_asset('images/logo.png');
                break;
            case 'favicon':
                ...
                break;
            case 'loader':
                $media = secure_asset('images/loader.gif');
                break;
            default:
                $media = secure_asset('images/default.png');
                break;
...
function getPriceFormat($price)
{
    ...
    $formattedPrice = number_format((float) $price, $afterdecimalpoint, ',', '.');

    if ($position == 'left') {
        $price = $symbol . " " . $formattedPrice;
    } else {
        $price = $formattedPrice . " " . $symbol;
    }
...
function getvnpaypayments($data)
{
    ...
    //     "vnp_Amount" => 1000000,
    //     "vnp_Command" => "pay",
    //     "vnp_CreateDate" => 20240912153750,
    //     "vnp_ExpireDate" => 20240912155250,
    //     "vnp_CurrCode" => "VND",
    //     "vnp_IpAddr" => "104.28.237.72",
    //     "vnp_Locale" => 'vn',
    //     "vnp_OrderInfo" => 'Thanh+toan+don+hang+thoi+gian',
    //     "vnp_OrderType" => 'topup',
    //     "vnp_ReturnUrl" => $vnp_Returnurl,
    //     "vnp_TxnRef" => $vnp_TxnRef,
    // );
...
function getmomopayments($data)
{
    $momo_key_data = getPaymentMethodkey('momo');

    $momo_Url = $momo_key_data['momo_url'];
    $accessKey = $momo_key_data['momo_accesskey'];
    $secretKey = $momo_key_data['momo_secretKey'];
    ...
    try {
        $response = Http::post('https://' . $momo_Url . '/create', ($requestBody));
