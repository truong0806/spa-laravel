...
class PaymentGatewayController extends Controller
{
    public function paymentPage(Request $request)
    {
        ...
        switch ($tabpage) {
            ...
            case 'vnpay':
                if (!empty($payment_data['value'])) {
                    $decodedata = json_decode($payment_data['value']);
                    $payment_data['vnpay_url'] = $decodedata->vnpay_url;
                    $payment_data['vnpay_key'] = $decodedata->vnpay_key;
                    $payment_data['vnpay_publickey'] = $decodedata->vnpay_publickey;
                }
                $data = view('paymentgateway.' . $tabpage, compact('user_data', 'tabpage', 'payment_data'))->render();
                break;
    ...
    public function paymentsettingsUpdates(Request $request)
    {
        ...
        switch ($type) {
            ...
            case 'momo':
                    $config_data = [
                        'momo_url' => $data['momo_url'],
                        'momo_partnerCode' => $data['momo_partnerCode'],
                        'momo_accesskey' => $data['momo_accesskey'],
                        'momo_secretKey' => $data['momo_secretKey']
                    ];
                    break;
    
            case 'vnpay':
                    $config_data = [
                        'vnpay_url' => $data['vnpay_url'],
                        'vnpay_key' => $data['vnpay_key'],
                        'vnpay_publickey' => $data['vnpay_publickey']
                    ];
                    break;
