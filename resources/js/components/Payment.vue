      ...
      <form @submit="formSubmit">
        <template v-if="!isChildComponentVisible">
          ...
          <div class="d-flex align-items-center flex-wrap gap-3">
            ...
            <div class="form-check" v-if="isVNPAYEnabled">
              <input class="form-check-input" type="radio" name="payment_method" v-model="payment_method" id='vnpay' value='vnpay' :checked="payment_method == 'vnpay'" />
              <label class="form-check-label h6 fw-normal text-capitalize" for="vnpay">{{$t('messages.vnpay')}}</label>
            </div>
            <div class="form-check" v-if="isMOMOEnabled">
              <input class="form-check-input" type="radio" name="payment_method" v-model="payment_method" id='momo' value='momo' :checked="payment_method == 'momo'" />
              <label class="form-check-label h6 fw-normal text-capitalize" for="momo">{{$t('messages.momo')}}</label>
            </div>
...
import { GET_PAYMENT_METHOD, GET_STRIPE_PAYMENT_URL, GET_VNPAY_PAYMENT_URL, GET_MOMO_PAYMENT_URL, WALLET_PAYMENT_API, PAYMENT_GATEWAY_LIST } from '../data/api';
import Swal from 'sweetalert2';
import { confirmcancleSwal, confirmcancleWallet } from '../data/utilities';
import Wallet from '../components/Wallet.vue';

const props = defineProps(['booking_id', 'customer_id', 'discount', 'total_amount', 'advance_payment_amount', 'wallet_amount','advance_paid_amount', 'is_pay_now']);
...
const isVNPAYEnabled = computed(() => {
  return Array.isArray(paymentGatewayList.value) && paymentGatewayList.value.some(gateway => gateway.type === 'vnpay' && gateway.status === 1);
});
const isMOMOEnabled = computed(() => {
  return Array.isArray(paymentGatewayList.value) && paymentGatewayList.value.some(gateway => gateway.type === 'momo' && gateway.status === 1);
});
...
const formSubmit = handleSubmit(async (values) => {
  ...
  if (values.payment_type == 'wallet') {
  ...
  } else if (values.payment_type == 'vnpay') {
    IsLoading.value = 1;
    const csrfToken = document.querySelector('meta[name="csrf-token"]').content;
    if (values.advance_payment_amount < 1000) {
      IsLoading.value = 0;
      Swal.fire({
        title: 'Error',
        text: 'Minimum Amount is 1000',
        icon: 'error',
        iconColor: '#5F60B9'
      }).then((result) => {});
    } else {
      ...
      if (response.ok) {
        ...
        const responseData = await response.json();
        if (responseData.payment_geteway_data != null) {
          Openvnpaypayment(responseData);
        } else {
          IsLoading.value = 0;
          Swal.fire({
            title: 'Error',
            text: 'Check Your VNPAY key Integration!',
            icon: 'error',
            iconColor: '#5F60B9'
          }).then((result) => {});
        }
  ...
  } else if (values.payment_type == 'momo') {
    IsLoading.value = 1;
    const csrfToken = document.querySelector('meta[name="csrf-token"]').content;
    const response = await fetch(GET_PAYMENT_METHOD, {
      method: 'POST',
    ...
    if (response.ok) {
      ...
      const responseData = await response.json();
      if (responseData.payment_geteway_data != null) {
        Openmomopayment(responseData);
      } else {
        IsLoading.value = 0;
        Swal.fire({
          title: 'Error',
          text: 'Check Your MoMo key Integration!',
          icon: 'error',
          iconColor: '#5F60B9'
        }).then((result) => {});
      }
...
const Openvnpaypayment = async (data) => {
  const csrfToken = document.querySelector('meta[name="csrf-token"]').content;

  const res = await fetch(GET_VNPAY_PAYMENT_URL, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-CSRF-TOKEN': csrfToken,
...
const Openmomopayment = async (data) => {
  const csrfToken = document.querySelector('meta[name="csrf-token"]').content;
  const res = await fetch(GET_MOMO_PAYMENT_URL, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-CSRF-TOKEN': csrfToken,
