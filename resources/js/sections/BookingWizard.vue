<template>
  <div class="row">
              ...
              <div class="d-sm-flex align-items-center gap-3 justify-content-between">
                ...
                <div class="flex-shrink-0 d-inline-flex align-items-center gap-2 mt-sm-0 mt-2">
                  <span class="text-primary fw-500 d-inline-block position-relative h5"
                    ...
                    ><span v-else>Free</span></span
                  >
                  <span class="font-size-18" v-if="service.duration">/</span>
                  <span class="h5 text-body" v-if="service.duration">{{
                ...
                <div class="d-flex align-items-center gap-1 flex-shrink-0">
                  ...
                  <h6 class="font-size-14">
                    {{ service.total_rating
                      ...
                      ({{ service.total_reviews }} {{ $t('messages.reviews') }})</span
                    >
                  </h6>
                </div>
              </div>
      ...
      <div class="bg-light p-sm-5 p-3 mb-5 booking-detail-service-box rounded-4">
        <div class="row align-items-center">
          <div class="col-lg-3 col-md-4">
            <div class="img flex-shrink-0">
              <img
                :src="service.package_image"
                class="object-cover rounded-3 w-100 img-fluid"
                alt="service"
              />
            </div>
          </div>
          <div class="col-lg-9 col-md-8 mt-md-0 mt-3">
            <div class="content flex-grow-1">
              <div class="d-sm-flex align-items-center gap-3 justify-content-between">
                <h4 class="mb-0">{{ service.name }}</h4>
                <div class="flex-shrink-0 d-inline-flex align-items-center gap-2 mt-sm-0 mt-2">
                  <span class="text-primary fw-500 d-inline-block position-relative font-size-18">{{
                    formatCurrencyVue(service.price)
                  }}</span>
                </div>
              </div>
            </div>
              ...
              <div :class="service.price == 0 ? 'col-lg-12' : 'col-lg-8'">
                <div class="booking-list-content-active">
                  ...
                  <div v-if="serviceaddon && serviceaddon.length">
                    ...
                    <div
                      v-for="(service, index) in serviceaddon"
                      :key="index"
                      class="mb-4 pb-4 d-flex align-items-sm-center aling-items-start flex-sm-row flex-column gap-5"
                    >
                      <div class="flex-shrink-0 provider-image-container">
                        <img
                          :src="service.serviceaddon_image"
                          alt="service-image"
                      ...
                      <div>
                        <span
                          class="d-block btn btn-light text-danger"
                          @click="removeAddons(index)"
                          >{{ $t('landingpage.remove') }}</span
                        >
                      </div>
                    </div>
                  </div>
                            ...
                            <div>
                              <div class="px-4 pt-3 pb-4 bg-body">
                                <div class="select-week-days">
                                  <div class="custom-form-field">
                                    ...
                                    <DatePicker
                                      ...
                                      mode="DateFormate"
                                      :min-date="new Date()"
                                      @click="handleDateSelect(DateFormate)"
                                      expanded
                                    />

                                    <span v-if="errorMessages['date']">
                                      <ul class="text-danger">
                                        <li v-for="err in errorMessages['date']" :key="err">
                                          {{ err }}
                                        </li>
                                      </ul>
                                    </span>
                                ...
                                <div v-if="date == null" class="time-slot mt-3 pt-3 border-top">
                                  <p class="text-capitalize mb-2 lh-1">
                                    {{ $t('landingpage.date_time') }}
                                  </p>

                                  <div
                                    v-for="(dayInfo, index) in availableserviceslot"
                                    :key="index"
                                  >
                                    <div v-if="dayInfo.day === dayName">
                                      <div v-if="dayInfo.slot != null">
                                        <ul
                                          class="list-inline m-0 d-flex align-items-center flex-wrap gap-3"
                                        >
                                          <li
                                            class="time-slot"
                                            v-for="timeSlot in dayInfo.slot"
                                            :key="timeSlot"
                                          >
                                            ...
                                            <label
                                              :for="timeSlot"
                                              class="btn d-block py-2 px-2 time-slot-btn"
                                            >
                                              {{
                                                timeSlot && timeSlot.match(/^(\d{2}):\d{2}:\d{2}$/)
                                                  ? RegExp.$1 + ':00'
                                                  : 'Invalid Time'
                                              }}
                                            </label>
                            ...
                            <span class="input-group-text flex-shrink-0" id="dateandtime">
                              ...
                              >
                                ...
                                <path
                                  fill-rule="evenodd"
                                  clip-rule="evenodd"
                                  d="M11.1787 2.18433H4.82822C2.6257 2.18433 1.25 3.41128 1.25 5.6666V12.4538C1.25 14.7446 2.6257 15.9999 4.82822 15.9999H11.1718C13.3812 15.9999 14.75 14.7659 14.75 12.5106V5.6666C14.7569 3.41128 13.3882 2.18433 11.1787 2.18433Z"
                                  stroke="currentColor"
                                  stroke-width="1.5"
                                  stroke-linecap="round"
                                  stroke-linejoin="round"
              ...
              <div class="col-lg-4" v-if="service.price > 0">
                <div class="booking-list-content">
                  <div class="d-flex justify-content-between">
                    ...
                    <button
                      v-if="SeletedCouponId > 0"
                      class="d-block btn btn-sm py-2 btn-danger text-end"
                      @click="RemoveCoupon()"
                    >
                      {{ $t('landingpage.remove_coupon') }}
                    </button>
                  </div>
                  <div class="table-responsive mt-5">
                    <table class="table">
                      <tbody>
                        <tr>
                          <td class="ps-0">
                            <span class="text-capitalize">{{ $t('messages.Price') }}</span>
                          </td>
                          <td class="pe-0">
                            <span class="d-block text-primary text-end">{{
                              formatCurrencyVue(service.price * quantity)
                            }}</span>
                          </td>
                        </tr>
                        <tr v-if="discount > 0">
                          <td class="ps-0">
                            <span class="text-capitalize"
                              >{{ $t('messages.discount') }} ({{ service.discount }}% off)</span
                            >
                          </td>
                          <td class="pe-0">
                            <span class="d-block text-success text-end"
                              >-{{ formatCurrencyVue(discount) }}</span
                            >
                          </td>
                        </tr>
                        ...
                        <tr
                          v-if="
                            coupons != '' &&
                            SeletedCouponId == 0 &&
                            props.service.package_type == null
                          "
                        >
                          <td class="ps-0">
                          ...
                          <td class="pe-0">
                            <span
                              class="d-block text-primary text-end cursor-pointer"
                              @click="OpenCouponCardMethod()"
                            >
                              {{ $t('messages.apply_coupon') }}</span
                            >
                          </td>
                        </tr>
                        ...
                        >
                          <td class="ps-0">
                            <span class="text-capitalize" @click="OpenCouponCardMethod()"
                              >{{ $t('landingpage.coupon') }} ({{ selectedCoupon.code }})</span
                            >
                          </td>
                          <td class="pe-0">
                            <span class="d-block text-success text-end">{{
                              formatCurrencyVue(coupondiscount)
                            }}</span>
                          </td>
                        </tr>
                        ...
                        <tr v-if="serviceaddon">
                          <td class="ps-0">
                            <span class="">{{ $t('landingpage.Add-ons') }}</span>
                          </td>
                          <td class="pe-0">
                            <span class="d-block text-primary text-end">{{
                              formatCurrencyVue(addonAmount)
                            }}</span>
                          </td>
                        </tr>
                        ...
                        <tr>
                          <td class="ps-0">
                            <span class="text-capitalize">{{ $t('landingpage.subtotal') }}</span>
                          </td>
                          <td class="pe-0">
                            <span class="d-block text-primary text-end">{{
                              formatCurrencyVue(subtotal || 0)
                            }}</span>
                          </td>
                        </tr>
                        ...
                        <tr>
                          <td class="ps-0">
                            <span class="text-capitalize">{{ $t('landingpage.tax') }}</span>
                          </td>
                          <td class="pe-0">
                            <span class="d-block text-danger text-end"
                              ><i
                                v-if="taxAmount > 0"
                                class="fa fa-info-circle text-body cursor-pointer"
                                aria-hidden="true"
                                @click="openTaxModel()"
                              ></i>
                              +{{ formatCurrencyVue(taxAmount || 0) }}</span
                            >
                          </td>
                        </tr>
                  ...
                  <div class="mt-5 pt-md-5 pt-3 text-md-end">
                    <div class="d-inline-flex align-items-center flex-wrap gap-3">
                      <a
                        :href="`${baseUrl}/service-detail/${service.id}`"
                      ...
                      <button
                        type="submit"
                        v-if="service.is_enable_advance_payment == 1"
                      ...
                      >
                        <span
                          ...
                          aria-hidden="true"
                        ></span
                        ><span v-else>{{ $t('landingpage.pay_advance') }}</span>
                      </button>
                      <button type="submit" v-else class="btn btn-primary">
                        <span
                          ...
                          aria-hidden="true"
                        ></span
                        ><span v-else>{{ $t('messages.book_now') }}</span>
                      </button>
                    </div>
                  </div>
                </div>
    ...
    <div
      class="modal fade show couponmodal"
      id="taxModal"
      v-if="is_tax == 1"
      tabindex="-1"
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="d-flex justify-content-between" v-for="tax in taxes" :key="tax">
              <p>{{ tax.title }}</p>
              <p v-if="tax.type == 'percent'">
                {{ formatCurrencyVue((tax.value * subtotal) / 100) }}
              </p>
...
const calculateAddonAmount = () => {
  addonAmount.value = props.serviceaddon.reduce((total, addon) => total + addon.Advance , 0)
}
...
const subtotal = computed(() => {
  if (coupondiscount.value > 0) {
    if (props.serviceaddon) {
      return (
        props.service.price * quantity.value +
        addonAmount.value -
        discount.value -
...
const formSubmit = handleSubmit(async (values) => {
  ...
  confirmcancleSwal({ title: title, subtitle: subtitle }).then(async (result) => {
    ...
    Swal.fire({
      ...
      html: 'Please wait, processing your booking...',
